...
def create_timeline_plot(video_path, output_path, skip=None, target_width=1920, max_width=None, waveform_samples=None, hwaccel=None):
    ...
    try:
        ...
        # Extract waveform
        waveform = extract_waveform_data(video_path, samples=waveform_samples)
        ...
        gs = GridSpec(2, 1, height_ratios=[2, 1], hspace=0.02)
        ...
        if waveform is not None and len(waveform) > 0:
            ...
            ax2.fill_between(time_axis, waveform, alpha=0.6, color='steelblue', label='Waveform')
            ax2.plot(time_axis, waveform, color='darkblue', linewidth=0.5, alpha=0.8)
            ax2.axhline(y=0, color='gray', linestyle='--', linewidth=0.5, alpha=0.5)
            ax2.set_ylim(-1.1, 1.1)
            ax2.set_ylabel('Amplitude', fontsize=11)
        else:
            ax2.text(duration/2, 0, 'No audio track', ha='center', va='center', 
...
def process_folder(folder_path, output_dir, skip=None, target_width=1920, max_width=None, waveform_samples=None, hwaccel=None):
    ...
    for idx, video in enumerate(videos, 1):
        ...
        try:
            create_timeline_plot(video_path, output_path, skip=skip, target_width=target_width,
                               max_width=max_width, waveform_samples=waveform_samples, hwaccel=hwaccel)
